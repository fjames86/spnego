...
/** this defines an HTTP server that requires Negotiate (SPNEGO) authentication. Point the client
 *  defined in client.lisp at it. Works for me!
 */
...
static ULONG InitVersion2( void ) {
	...
	authinfo.AuthSchemes = HTTP_AUTH_ENABLE_NEGOTIATE; //HTTP_AUTH_ENABLE_BASIC; //HTTP_AUTH_ENABLE_NTLM;
...
void version2( void ) {
	...
	//-2146893048 == 0x80090308L == SEC_E_INVALID_TOKEN
	if( ainfo && ainfo->AuthStatus == HttpAuthStatusSuccess ) {		
		...
		printf( "(%s) Authentication failed: %08x\n", buff, ainfo->SecStatus );
		count = sizeof(HTTP_RESPONSE) + 2*1024;
		resbuffer = (HTTP_RESPONSE *)calloc( 1, count );
		resbuffer->EntityChunkCount = 1;
		ZeroMemory( &datachunk, sizeof(datachunk) );
